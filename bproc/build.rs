...
fn main() {
    ...
    if env::var("CARGO_FEATURE_THROTTLE_EBPF").is_ok() {
        println!("cargo:rerun-if-changed=../bproc-ebpf/src");
        ...
        if egress_exists && ingress_exists && tc_classifier_exists {
            ...
            let copy_egress = std::fs::copy(
                target_dir.join("bproc-egress"),
                out_dir.join("bproc-egress"),
            );
            let copy_ingress = std::fs::copy(
                target_dir.join("bproc-ingress"),
                out_dir.join("bproc-ingress"),
            );
            let copy_tc_classifier = std::fs::copy(
                target_dir.join("bproc-tc-classifier"),
                out_dir.join("bproc-tc-classifier"),
            );
