        ...
        if self.ebpf.is_none() {
            ...
            #[cfg(ebpf_programs_built)]
            {
                ...
                const PROGRAM_BYTES: &[u8] =
                    aya::include_bytes_aligned!(concat!(env!("OUT_DIR"), "/bproc-egress"));
                ...
                let program: &mut CgroupSkb = ebpf
                    .program_mut("bproc_egress")
                    .ok_or_else(|| anyhow::anyhow!("Program bproc_egress not found"))?
                    .try_into()
                    .context("Program is not a CgroupSkb program")?;

                program
                    .load()
                    .context("Failed to load bproc_egress program into kernel")?;
                log::info!("✅ Loaded bproc_egress program into kernel (maps created)");
