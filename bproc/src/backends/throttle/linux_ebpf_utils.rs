...
pub fn get_cgroup_id(pid: i32) -> Result<u64> {
    // DIAGNOSTIC: If BPROC_TEST_ROOT_CGROUP is set, always return root cgroup ID
    if std::env::var("BPROC_TEST_ROOT_CGROUP").is_ok() {
...
pub fn get_cgroup_path(pid: i32) -> Result<PathBuf> {
    // DIAGNOSTIC: If BPROC_TEST_ROOT_CGROUP is set, use root cgroup for testing
    if std::env::var("BPROC_TEST_ROOT_CGROUP").is_ok() {
...
impl BpfAttachMethod {
    ...
    pub fn from_env_and_arg(arg: Option<&str>) -> Self {
        ...
        if std::env::var("BPROC_USE_LEGACY_ATTACH").is_ok() {
            return Self::Legacy;
        }

        // Check new environment variable
        if let Ok(method) = std::env::var("BPROC_BPF_ATTACH_METHOD") {
            return Self::from_str(&method);
        }
