...
//! # How it works
//!
//! 1. Create cgroup at `/sys/fs/cgroup/net_cls/bproc/<name>`
...
impl CgroupV1Backend {
    pub fn new() -> Result<Self> {
        let base_path = PathBuf::from(CGROUP_V1_BASE).join(BPROC_CGROUP);
        Ok(Self {
            base_path,
...
impl CgroupBackend for CgroupV1Backend {
    ...
    fn create_cgroup(&self, pid: i32, name: &str) -> Result<CgroupHandle> {
        ...
        fs::create_dir_all(&self.base_path).context("Failed to create bproc cgroup directory")?;
