...
//! # How it works
//!
//! 1. Create cgroup at `/sys/fs/cgroup/bproc/<name>`
//! 2. Write PID to `cgroup.procs`
//! 3. nftables rule matches with: `socket cgroupv2 "/sys/fs/cgroup/bproc/pid_1234"`
...
impl CgroupV2NftablesBackend {
    pub fn new() -> Result<Self> {
        let base_path = PathBuf::from(CGROUP_V2_BASE).join(BPROC_CGROUP);
        Ok(Self { base_path })
    }
...
impl CgroupBackend for CgroupV2NftablesBackend {
    ...
    fn create_cgroup(&self, pid: i32, name: &str) -> Result<CgroupHandle> {
        // Create base bproc cgroup if needed
