// macOS dummynet + PF download throttling backend
//
// Uses:
...
// 4. Load rules: dummynet in on <iface> proto tcp from <ip> port <port> to <ip> port <port> pipe N
//
// Limitations:
...
/// macOS dummynet download (ingress) throttling backend
pub struct DnctlDownload {
...
impl DnctlDownload {
    pub fn new() -> Result<Self> {
        ...
        Ok(Self {
            ...
            next_pipe: 200, // Start at 200 to avoid conflicts with upload (100+)
            process_utils,
            initialized: false,
    ...
    fn generate_pf_rule(&self, conn: &ConnectionInfo, pipe_num: u32) -> String {
        // Format: dummynet in on <iface> proto <proto> from <remote> port <rport> to <local> port <lport> pipe <N>
        // NOTE: For ingress traffic, packets come FROM remote TO local
        // macOS uses 'dummynet' keyword (not FreeBSD's 'dnpipe')
        format!(
            "dummynet in on {} proto {} from {} port {} to {} port {} pipe {}",
            self.interface,
            conn.protocol,
            conn.remote_addr, // Ingress: from REMOTE
            conn.remote_port,
            conn.local_addr, // Ingress: to LOCAL
            conn.local_port,
            pipe_num
        )
    }

    /// Load PF rules using anchor (proper macOS approach)
    fn load_pf_rules(&self, rules: &[String]) -> Result<()> {
        ...
        eprintln!("DEBUG [Download]: PF rules being loaded:");
        eprintln!("---START---");
        eprintln!("{}", rules_text);
        eprintln!("---END---");
        log::info!(
            "DEBUG [Download]: Loading {} rules: {}",
            rules.len(),
            rules_text
        );
...
fn load_pf_rules(rules: &[String]) -> Result<()> {
    ...
    // DEBUG: Print exact rules being sent to pfctl
    eprintln!("DEBUG [Download/Standalone]: PF rules being loaded:");
    eprintln!("---START---");
    eprintln!("{}", rules_text);
    eprintln!("---END---");
    log::info!(
        "DEBUG [Download/Standalone]: Loading {} rules: {}",
        rules.len(),
        rules_text
    );
...
impl DownloadThrottleBackend for DnctlDownload {
    ...
    fn init(&mut self) -> Result<()> {
        ...
        log::info!(
            "Initialized macOS dnctl download backend on interface {}",
            self.interface
        );
        Ok(())
    }
    ...
    fn throttle_download(
        ...
        log::info!(
            "Download throttle applied: PID {} ({}) → {} bytes/sec (pipe {})",
            pid,
            process_name,
            limit_bytes_per_sec,
            pipe_num
    ...
    fn remove_download_throttle(&mut self, pid: i32) -> Result<()> {
        ...
        log::info!(
            "Download throttle removed: PID {} ({}) pipe {}",
            pid,
            state.process_name,
            state.pipe_num
        );
    ...
    fn cleanup(&mut self) -> Result<()> {
        log::info!("Cleaning up macOS dnctl download backend...");
        ...
        log::info!("macOS dnctl download backend cleanup complete");
        Ok(())
    }
}

impl Drop for DnctlDownload {
    fn drop(&mut self) {
        // Signal shutdown
        self.shutdown_flag.store(true, Ordering::Relaxed);
        ...
        if let Err(e) = self.cleanup() {
            log::error!("Error during DnctlDownload cleanup: {}", e);
        }
    }
}
