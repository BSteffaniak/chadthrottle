...
/// Classify the remote address relative to the cgroup-SKB network header.
#[inline(always)]
fn should_throttle_packet(ctx: &SkBuffContext, traffic_type: u8) -> bool {
    bproc_common::traffic::should_throttle_ip_packet(traffic_type, true, |offset, out| {
        ctx.load_bytes(offset, out)
            .is_ok_and(|read| read == out.len())
    })
}
...
#[cgroup_skb(ingress)]
pub fn bproc_ingress(ctx: SkBuffContext) -> i32 {
    match try_bproc_ingress(ctx) {
        Ok(ret) => ret,
        Err(_) => 1, // Allow on error
    }
}

fn try_bproc_ingress(ctx: SkBuffContext) -> Result<i32, i64> {
