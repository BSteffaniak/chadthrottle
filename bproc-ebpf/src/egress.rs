...
fn should_throttle_packet(ctx: &SkBuffContext, traffic_type: u8) -> bool {
    bproc_common::traffic::should_throttle_ip_packet(traffic_type, false, |offset, out| {
...
#[cgroup_skb(egress)]
pub fn bproc_egress(ctx: SkBuffContext) -> i32 {
    match try_bproc_egress(ctx) {
        Ok(ret) => ret,
        Err(_) => 1, // Allow on error
    }
}

fn try_bproc_egress(ctx: SkBuffContext) -> Result<i32, i64> {
