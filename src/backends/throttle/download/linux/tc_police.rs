...
impl DownloadThrottleBackend for TcPoliceDownload {
    ...
    fn throttle_download(
        ...
        log::warn!(
            ...
            limit_bytes_per_sec, self.interface
        );
        ...
        let status = Command::new("tc")
            .args(&[
                ...
                "prio", "1",
                "u32", "match", "u32", "0", "0", // Match all traffic
                "police", "rate", &format!("{}bit", rate_bps),
                "burst", &format!("{}k", (rate_bps / 8000).max(32)), // Reasonable burst size
                "drop", // Drop packets exceeding rate
                "flowid", &format!(":{}", handle),
            ])
            .status()
